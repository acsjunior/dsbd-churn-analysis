...
Definido o valor de \textit{cutoff}, através do cruzamento dos valores preditos pelo modelo e os valores observados, foi construída a matriz de confusão (Tabela \@ref(tab:cmatrix)), que apresenta em sua diagonal principal o número de classificações corretas e, na diagonal secundária, o número de classificações incorretas. A partir da matriz de confusão foram calculadas as seguintes métricas de avaliação: sensibilidade (taxa de classificação correta entre as observações com a ocorrência de churn), especificidade (taxa de classificação correta entre as observações sem a ocorrência de churn) e acurácia (taxa global de classificações corretas), representadas pelas equações \@ref(eq:sensitivity), \@ref(eq:specificity) e \@ref(eq:accuracy), respectivamente. 
...
\begin{equation}
Sensibilidade = \dfrac{VP}{VP+FN}(\#eq:sensitivity)
\end{equation}

\begin{equation}
Especificidade = \dfrac{VN}{VN+FP}(\#eq:specificity)
\end{equation}

\begin{equation}
Acurácia = \dfrac{VN+VP}{VN+VP+FN+FP}(\#eq:accuracy)
\end{equation}
