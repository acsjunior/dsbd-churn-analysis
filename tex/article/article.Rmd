---
title: "Classificação de Churn Utilizando um Modelo de Regressão Logística Binária"
subtitle: "Customer Churn Classification Using a Binary Logistic Regression Model"
...
lang: pt-BR
resumo: >
  O desenvolvimento de estratégias para retenção de clientes se tornou uma prática comum entre companhias de diversos segmentos, uma vez que relacionamentos de longo prazo com clientes estão associados à sobrevivência econômica e ao sucesso das empresas. Portanto, com a intenção antever os clientes propensos a abandonar o relacionamento com uma startup brasileira, fenômeno conhecido como \textit{churn}, a proposta deste artigo é apresentar um modelo preditivo que possibilite não só a classificação de \textit{churn}, mas que também permita a interpretação dos motivos que possivelmente estejam impactando a predição. Após árduo processo de data wrangling, aplicou-se a regressão logística binária fazendo uso da validação cruzada K-fold e do algortimo \textit{stepwise} para seleção de covariáveis. O modelo final, composto por 14 covariáveis, passou por uma análise de diagnóstico por meio dos resíduos quantilicos aleatorizados e teve o poder preditivo avaliado através da curva ROC, matriz de confusão e de métricas de avaliação. Em todas as etapas do estudo o modelo foi considerado adequado para o negócio, e além de exibir um bom poder preditivo, demonstrou-se capaz de fornecer insights para ações de marketing personalizadas e otimizadas com foco na retenção dos clientes propensos a dar \textit{churn}.
palavras_chave: "Churn, Retenção de clientes, CRM, Regressão logística binária, Stepwise."
abstract: "Developing strategies for customer retention has become a common practice among companies from different segments, since long-term relationships with customers are associated to the economic survival and success of companies. Therefore, with the intention of predict the customers churn of a Brazilian startup, the purpose of this article is to present a predicitve model that allows to interpret the reasons that are possibly impact the prediction. After an arduous data wrangling proccess, the binary logistic regression was applied using K-fold cross-validation and the stepwise algorithm for covariate selection. The final model composed by 14 covariates has undergone a diagnostic analysis through randomized quantile residuals and its predict power was evatuated by the ROC curve, confusion matrix and evaluating metrics. The model was considered appropriate for the business in all stages of the study and besides proved its good predict power, it also demonstrated to be able to provide insights for optimized and customized marketing actions focused on retention of customers likely to churn."
keywords: "Customer churn, Customer retention, CRM, Binary logistic regression, Stepwise"
---
...
<!-- \begin{equation} -->
<!-- \dfrac{V2}{V1+V2} (\#eq:desempenho) -->
<!-- \end{equation} -->

As métricas desempenho, data à natureza da equação de origem, possuem o comportamento explicado pela Tabela \@ref(tab:metricas). Ao término desta etapa foi obtido um conjunto de dados composto pela variável resposta (\textit{churn}) e, como covariáveis, 9 métricas de desempenho, onde cada observação representa um vendedor.
...
A estimação dos parâmetros $\beta_j$ é realizada por máxima verossimilhança, método que consiste em encontrar, através da programação linear, os parâmetros que maximizam a função de verossimilhança representada através da Equação \@ref(eq:verossimilhanca).
\begin{equation}
...
names(dicionario) <- c("Covariável", "Descrição", "Medição")
...
# # odds$Unidade <- c(rep(0.5, 5), rep(1, 4), 100, 10, 1, 1, 1)
# odds$Chance = round(with(odds, exp(Estimativa)),2)
# odds$Variação <- round((odds$Chance - 1) * 100,0)
# 
# odds <- odds %>%
#   arrange(desc(abs(Variação))) 
# 
# odds %>% 
#   kable(col.names = c("Covariável", "Estimativa", "Chance", "Variação (%)"),
#         booktabs = T, caption = "Interpretação das chances de ocorrência de churn") %>%
#   kable_styling(font_size = TABLE_FONT_SIZE)
...
# Ao analisar a Tabela \@ref(tab:oddsnumericas), percebe-se que o número de dias em atividade no período (X31), o número de dias desde a contratação até a realização da primeira venda (X29), o valor total do faturamento (X22) e o número total de produtos publicados (X27) reduzem, em média, a chance de churn à medida que seus respectivos valores aumentam. Em contrapartida, o número de dias desde a contratação até o vendedor estar pronto para operar (X30) eleva, em média a chance de churn à medida que o seu valor aumenta.

# Quanto às covariáveis X14 e X11, a interpretação pode ser realizada de maneira análoga, ao passo que com relação à covariável X20, podemos interpretar que a chance de churn para os vendedores que estão localizados na região sudeste é 17% maior quando comparados com os vendedores de outras regiões, fixadas as demais covariáveis.

# Por meio da Tabela \@ref(tab:odds) é possível analisar e separar as estimativas dos parâmetros em dois grupos. O grupo com valores inferiores a zero representa as estimativas que reduzem, em média, a chance de ocorrência de churn em função do aumento do valor de sua respectiva covariável, ao passo que o grupo com valores superiores a zero representa as estimativas que elevam, em média, a chance de ocorrência de churn em função do aumento do valor de sua respectiva covariável.


# A Tabela \@ref(tab:odds) apresenta as estimativas dos parâmetros do modelo ordenadas decrescentemente em função da variação da chance de ocorrência do churn e sua interpretação ocorre da seguinte forma: A chance de churn fica multiplicada por $e^{0,9518}$ para cada 1 unidade a mais na métrica de desempenho do número de pedidos (X1), mantidas as demais condições constantes. Diante do fato de que esta métrica se apresenta no intervalo entre 0 e 1 e possui as características exibidas na Tabela \@ref(tab:metricas), é possível dizer que um vendedor com o desempenho do número de pedidos mantido (X1 = 0,5), fixadas as demais covariáveis, tem a chance de churn multiplicada por $e^{0,5 \cdot 0,9519}$. Em outras palavras, a chance de churn de um  
# 
# 
# As covariáveis X1, X17, X14, X15 e X34, por apresentarem as maiores variações, 
# 
# As covariáveis X14, X15, X17 e X34, por apresentarem as maiores estimativas, foram consideradas as mais importantes para o modelo e podem ser interpretadas da seguinte forma: A chance de churn fica multiplicada por $e^{-2,992}$ para os vendedores que estão no estágio I (X14) com relação aos que não estão no mesmo estágio. Em outras palavras, a chance de churn para os vendedores que estão no estágio I é 95% menor que a chance de churn dos vendedores que não estão no mesmo estágio. Seguindo a mesma linha de raciocínio, a chance de churn dos vendedores que estão no estágio R (X15) é 93% menor com relação aos vendedores que não estão no mesmo estágio e a chance de churn dos vendedores que contrataram o plano M (X17) é 99% menor com relação aos vendedores que não contrataram o mesmo plano. Quanto a covariável numérica X34, a chance de churn fica multiplicada por $e^{-1,6710}$ para 1 dia a mais de atividade ou ainda multiplicada por $e^{5 (-1,6710)}$ para 5 dias a mais de atividade. Em outras palavras, 1 e 5 dias a mais de atividade impactam na redução da chance de churn em 81% e 99%, respectivamente, fixadas as demais covariáveis.  
...
# df_train %>%
#   group_by(y) %>%
#   summarise(avg = mean(x29))
# 
# ggplot(df_train, aes(x = factor(y), y = x29)) +
#   geom_boxplot() +
...
```{r oddsnumericas}
var_numericas <- dicionario[dicionario$Medição %in% c("contínua","discreta"),]
var_numericas <- str_to_upper(var_numericas$Covariável)
odds_numericas <- odds[odds$Covariável %in% var_numericas,]
odds_numericas$Chance <- round(with(odds_numericas, exp(Estimativa)),4)
odds_numericas$Variação <- round((odds_numericas$Chance - 1) * 100, 0)

odds_numericas <- odds_numericas %>%
  arrange(desc(abs(Variação)))

kable(odds_numericas, booktabs = T, caption = "Chances de ocorrência de churn para covariáveis quantitativas",
      col.names = c("Covariável", "Estimativa", "Chance", "Variação (%)")) %>%
  kable_styling(font_size = TABLE_FONT_SIZE)
```

```{r oddsbinarias}
var_binarias <- dicionario[dicionario$Medição == "1 = sim; 0 = não",]
var_binarias <- str_to_upper(var_binarias$Covariável)
odds_binarias <- odds[odds$Covariável %in% var_binarias,]
odds_binarias$Chance <- round(with(odds_binarias, exp(Estimativa)),4)
odds_binarias$Variação <- round((odds_binarias$Chance - 1) * 100, 0)

odds_binarias <- odds_binarias %>%
  arrange(desc(abs(Variação)))

kable(odds_binarias, booktabs = T, caption = "Chances de ocorrência de churn para covariáveis qualitativas",
      col.names = c("Covariável", "Estimativa", "Chance", "Variação (%)")) %>%
  kable_styling(font_size = TABLE_FONT_SIZE)
```

```{r oddsmetricas}
var_metricas <- dicionario[dicionario$Medição == "percentual",]
var_metricas <- str_to_upper(var_metricas$Covariável)
odds_metricas <- odds[odds$Covariável %in% var_metricas,]
odds_metricas$Chance <- round(with(odds_metricas, exp(0.5 * Estimativa)),4)
odds_metricas$Variação <- round((odds_metricas$Chance - 1) * 100, 0)

odds_metricas <- odds_metricas %>%
  arrange(desc(abs(Variação)))

kable(odds_metricas, booktabs = T, caption = "Chances de ocorrência de churn para covariáveis de desempenho",
...
Para possibilitar a avaliação do poder preditivo do modelo na amostra de validação, é necessário antes definir o valor de \textit{cutoff}, ou seja, um ponto de corte de modo que as observações com probabilidade de ocorrência de \textit{churn} superior ao \textit{cutoff} sejam classificadas como \textit{churn} ($Y=1$) e, caso contrário, classificadas como não \textit{churn} ($Y=0$). A escolha do \textit{cutoff} foi realizada através da análise da curva de Sensibilidade, que é um gráfico que apresenta os valores da sensibilidade e da especificidade em função dos valores de \textit{cutoff}, e da curva ROC (\textit{Receiver Operating Characteristic}), gráfico que apresenta a variação da sensibilidade em função de (1 - especificidade), que mostra o comportamento do \textit{trade off} entre a sensibilidade e a especificidade em função da alteração do \textit{cutoff} \cite{Favero2017}. Atrávés do cálculo da área sob a curva ROC (AUC - \textit{Area Under the Curve}) é possível avaliar a eficiênca global do modelo, sendo $AUC=1$ o melhor valor possível. O modelo em estudo apresentou $AUC = 0,8894$, o que indica que o modelo possui uma boa eficiência global. Analisadas as curvas de Sensibilidade (Figura \@ref(fig:senscurve)) e ROC (Figura \@ref(fig:roc)) e considerando os requisitos do negócio, optou-se por um valor de \textit{cutoff} que garantisse um pouco mais de sensibilidade com relação à especificidade. Portanto, para continuidade do estudo foram consideradas como \textit{churn} (Y=1) as observações com probabilidade de ocorrência de \textit{churn} superior a 0,54.

```{r senscurve, fig.cap='Curva de Sensibilidade', out.width="100%", fig.align='center'}
...
# cutoff <- as.numeric(coords(rocobj, x = "best", best.method = "youden",
...
ggroc(rocobj, colour = "blue", size = 1) +
  ...
  # annotate("point", x = 0.8008186, y = 0.8406677, colour = "red", size = 4, alpha = 0.8) +
  # annotate(geom = "text", x = 0.69, y = 0.8406677, label = "Cutoff = 0,53", color = "red") +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14, face="bold"),
        title=element_text(size=16, face="bold"))
```
...
\begin{equation}
\dfrac{VP}{VP+FN}(\#eq:sensitivity)
\end{equation}

\begin{equation}
\dfrac{VN}{VN+FP}(\#eq:specificity)
\end{equation}

\begin{equation}
\dfrac{VN+VP}{VN+VP+FN+FP}(\#eq:accuracy)
\end{equation}
...
Ao Olist por disponibilizar os dados para o desenvolvimento do trabalho. Ao corpo docente da Especialização em Data Science e Big Data da Universidade Federal do Paraná - UFPR, pela qualidade do ensino oferecido, em especial ao  Prof. Walmes Zeviani pela orientação, inspiração e disponibilidade, inclusive nos finais de semana, e ao Prof. Cesar Taconeli, que através das suas excelentes aulas me inspirou o interesse por Modelos Lineares Generalizados. Ao Prof. Marcos Santos do Instituto Militar de Engenharia - IME, pelo suporte e incentivo à pesquisa.

```{r}
# ## Validação cruzada K-fold {#sec:kfoldcv}
# 
# A validação cruzada k-fold é uma técnica de reamostragem aplicada com o propósito de reduzir o vício e a variância do modelo, que funciona da seguinte forma \cite{Kohavi1995}:
# 
