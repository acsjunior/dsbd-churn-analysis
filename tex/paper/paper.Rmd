...
Quando a estatística do $TRV$ é inferior ao valor da distribuição do ${\chi}^2$ com $k$ graus de liberdade e 5% de significância, não rejeitamos a hipótese nula, ou seja, concluimos que a remoção de $k$ covariáveis não afeta a qualidade do ajuste do modelo.

## Resíduos quantílicos aleatorizados

Considerando um modelo de regressão linear bem ajustado, espera-se que os resíduos ordinários, definidos pela diferença entre os valores observados e estimados, estejam normalmente distribuídos. Entretanto, os resíduos ordinários de um modelo de regressão para dados não normais, como a regressão logística, não acompanham a distribuição normal, dificultando a análise gráfica. Com a intenção de generalizar o método de análise dos resíduos da regressão linear para todos os modelos lineares generalizados, Dunn e Smyth \cite{Dunn1996} propuseram os resíduos quantílicos aleatorizados, definidos por $r_i = \phi^{-1}(u_i)$, onde $\phi^{-1}$ é a inversa da função de distribuição acumulada da normal padrão e $u_i = F(y_i;\mu_i,\phi)$, com distribuição uniforme entre 0 e 1, é calculado com base na distribuição acumulada do modelo proposto. Portanto, caso o modelo logístico esteja bem ajustado, os resíduos quantílicos aleatorizados irão se apresentar normalmente distribuídos e com variância constante \cite{Taconeli2015}.

## Validação cruzada por K-fold

A validação cruzada por k-fold é uma técnica de reamostragem aplicada com o propósito de reduzir o vício e a variância do modelo, que funciona da seguinte forma \cite{LegReamostragem}:
